...
yükseklik (tam boy kareler), sağ iki sütun ortadan bölünmüş (dört portre karesi). Varsayılan
tuval 3840x2160'tır.
...
KADRAJ ESNEME PAYI: ölçülen büyüklük karakterin kadrajda kapladığı alandır - kafa genişliğinin
hücre genişliğine oranı. Üretilen kareler bu oranı birebir tutmaz ve tutmak zorunda da değildir;
%10'a kadar sapma tolere edilir. Pay içindeki kare yeniden ölçeklenmez, olduğu gibi alınıp
yalnızca baş üstü boşluğuna göre hizalanır. Pay dışındaki makul sapmalar sessizce hedef kafa
ölçüsüne getirilir. Yalnızca kadrajın gerçekten bozuk olduğu durumda (KADRAJ_SINIR ötesi) uyarı
basılır.
...
# Kadraj esneme payı. Ölçüt mutlak ölçek değil, hizalama ölçeğinin hücreyi kaplayan ölçekten
# sapmasıdır - kaynak karenin piksel boyutundan bağımsız olsun diye. Kadrajı doğru üretilmiş
# bir karede bu ikisi eşittir.
KADRAJ_TOLERANS = 0.10         # bu payın içindeki sapmaya dokunulmaz
KADRAJ_SINIR = 1.60            # bu katsayının ötesi bozuk kadrajdır, uyarı basılır
...
def kaplama_olcegi(a, hucre_w, hucre_h):
    """Kareyi hücreyi boşluksuz dolduracak en küçük ölçek."""
    h, w = a.shape[:2]
    return max(hucre_w / w, hucre_h / h)
...
def kirp(a, hucre_w, hucre_h, olcek=None, tepe=None, ust_bosluk=None):
    ...
    h, w = a.shape[:2]
    kapla = kaplama_olcegi(a, hucre_w, hucre_h)
...
def kadraj_olcegi(a, hucre_w, hucre_h):
    """Portre karesi için hizalama ölçeğini seçer.

    (ölçek, tepe y, not, uyarı) döndürür. Ölçek None ise kare ortalanır. Karar, karakterin
    kadrajda kapladığı oranın hedeften sapmasına bakılarak verilir - bkz. KADRAJ_TOLERANS."""
    olcum = kafa_olcusu(a)
    if olcum is None:
        return None, None, "", "kafa ölçülemedi - hizalama yapılmadı, kare ortalandı"

    kafa, tepe = olcum
    kapla = kaplama_olcegi(a, hucre_w, hucre_h)
    hedef = (hucre_w * PORTRE_KAFA) / max(1.0, kafa)
    sapma = hedef / kapla
    yuzde = round((sapma - 1) * 100)

    if abs(sapma - 1) <= KADRAJ_TOLERANS:
        return kapla, tepe, f"kadraj %{yuzde:+d} - pay içinde", ""
    if 1 / KADRAJ_SINIR <= sapma <= KADRAJ_SINIR:
        return hedef, tepe, f"kadraj %{yuzde:+d} - hizalandı", ""
    return None, None, "", f"kadraj ölçeği pay dışı (%{yuzde:+d}) - kare yeniden üretilmeli"
...
def kareyi_hazirla(yol, hucre_w, hucre_h, portre):
    ...
    if portre:
        istenen, tepe, not_, uyari = kadraj_olcegi(a, hucre_w, hucre_h)
        if uyari:
...
def main():
    ...
    cikti = a.cikti or f"{a.ad} - Karakter Referans Sayfası.png"
    sayfa.save(cikti)
