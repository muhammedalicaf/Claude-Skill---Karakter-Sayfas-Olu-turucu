...
GREN ÖLÇEKLEME: kalibrasyon çifti 2048 piksel genişliğindeydi ve gren tane boyutu piksel
cinsinden ölçüldü. Lightroom'da tane boyutu görüntünün kendi ölçüsüne göredir; 3840 piksellik
bir sayfaya aynı piksel sigmasını uygulamak taneyi görece yarı yarıya inceltir. Bu yüzden
sigma, görüntü genişliğinin kalibrasyon genişliğine oranıyla çarpılır. `--kalibrasyon-genislik 0`
ölçeklemeyi kapatır ve v1 davranışını birebir verir. Renk ve ton boru hattı bundan etkilenmez.
...
KALIBRASYON_GENISLIK = 2048  # ölçümün yapıldığı görüntü genişliği; gren tane boyutu buna göre
...
def gren_uygula(y, miktar, boyut, puruzluluk, tohum=0, olcek=1.0):
    """Tek kanallı (parlaklık) gren. Boyut tane inceliğini, pürüzlülük ince/kaba karışımını verir.

    olcek, tane boyutunu kalibrasyon genişliğinden bugünkü görüntü genişliğine taşır."""
    if miktar <= 0:
        return y
    rng = np.random.default_rng(tohum)
    h, w = y.shape[:2]
    sigma = (0.25 + (boyut / 100) * 1.4) * olcek        # boyut 25, olcek 1 -> ~0.6 px
...
def uygula(a, pozlama=-0.40, kontrast=-4, sicaklik=-4, ton=-4,
           ...
           gren_puruzluluk=50, tohum=0, kalibrasyon_genislik=KALIBRASYON_GENISLIK):
    x = a.astype(np.float64) / 255
    gren_olcek = a.shape[1] / kalibrasyon_genislik if kalibrasyon_genislik else 1.0
    y = l2s(s2l(x) * kanal_kazanclari(pozlama, sicaklik, ton))
    y = s_egrisi(y, kontrast)
    y = doygunluk_uygula(y, canlilik, doygunluk)
    y = gren_uygula(y, gren_miktar, gren_boyut, gren_puruzluluk, tohum, gren_olcek)
...
def main():
    ...
    p.add_argument("--kalibrasyon-genislik", type=int, default=KALIBRASYON_GENISLIK,
                   help="gren tane boyutunun ölçeklendiği referans genişlik; 0 ölçeklemeyi kapatır")
    ...
    sonuc = uygula(kaynak, a.pozlama, a.kontrast, a.sicaklik, a.ton,
                   ...
                   a.gren_puruzluluk, a.tohum, a.kalibrasyon_genislik)
